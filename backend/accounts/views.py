...
# # ============================================
# # GET ALL PROFESSIONALS (TECHNICIANS)
# # ============================================
# @api_view(['GET'])
# @permission_classes([AllowAny])
# def get_all_professionals(request):
#     """Get all verified technicians/professionals"""
#     try:
#         technicians = TechnicianProfile.objects.filter(is_approved=True, is_available=True)
#         data = []
...
# @api_view(['GET'])
# @permission_classes([AllowAny])
# def get_professionals_by_service(request):
...
#         }
#     })
# # "@ | Out-File -FilePath "accounts\views.py" -Encoding UTF8
...
#             message=f'New booking request from {request.user.first_name} {request.user.last_name} for {data["service_type"]}. OTP: {booking.id}'
#         )
...
#             'status': booking.status,
#             'otp': booking.id  # Using booking ID as OTP for demo
#         }, status=status.HTTP_201_CREATED)
